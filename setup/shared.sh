...
setup_python_venv() {
    ...
    if [ -n "$req_file" ]; then
        ...
        if [ "$req_file" != "$PROJECT_ROOT/requirements-provisioning.txt" ]; then
            ...
            local sdk_dir="$PROJECT_ROOT/../jarvis-command-sdk"
            if [ -d "$sdk_dir" ]; then
                log_info "Installing jarvis-command-sdk..."
                $use_sudo "$venv_path/bin/python" -m pip install -e "$sdk_dir" --quiet
                log_success "jarvis-command-sdk installed"
            fi
...
setup_database() {
    ...
    if [ -f "$PROJECT_ROOT/alembic.ini" ]; then
        ...
        if [ "$backup_count" -gt 3 ]; then
            ls -1t "${db_file}.bak."* | tail -n +4 | xargs rm -f
            log_info "Cleaned up old database backups (kept last 3)"
        fi
    else

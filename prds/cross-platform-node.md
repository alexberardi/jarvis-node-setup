...
| **2 — Decouple audio devices from the HAT** | Configurable playback device (capture already is); gate TLV320/ReSpeaker recovery + keepalive + LED behind the hardware-capability flag; parametrize `MIC_RATE` (`voice_listener.py:79`) from `mic_sample_rate` + rational resample. *Shared by HAT-less Pi + container.* | `core/platform_abstraction.py`, `utils/audio_volume.py`, `utils/mic_device.py`, `scripts/voice_listener.py`, `core/wake_loop.py` | M |
| **3 — Container audio image (no AEC), multi-arch** | Split `requirements-hat.txt` out of `requirements-audio.txt`; Dockerfile audio layer (portaudio/speexdsp/pulse-utils + base+audio, `openwakeword --no-deps`); `JARVIS_VOICE_MODE=voice` entrypoint branch; `ContainerAudioProvider`/`LinuxHostAudioProvider` overriding `play_pcm_stream`; add `platforms: linux/amd64,linux/arm64`. | `Dockerfile`, `requirements-*.txt`, `scripts/entrypoint.py`, `core/platform_abstraction.py`, `release.yml` | M–L |
| **4 — Setup script + compose wiring** | `configure-audio.sh` (enumerate → pick → write `audio.env`); `docker-compose.audio.yaml`; non-root-uid rework (relocate `/root/.jarvis` mounts); macOS→native steering. | new script, new compose, `Dockerfile` (USER), mount layout | M |

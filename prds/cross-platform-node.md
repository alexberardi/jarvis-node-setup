...
| **6 — AEC in container** ✅ | **Validated working in-container** (no code change to the AEC core needed — the existing `parec`-on-`.monitor` path works over the shared pulse socket). The `.monitor` open question is resolved: it streams real audio during playback. Enable with `JARVIS_AEC_ENABLED=true` (needs the pulse overlay for the reference). Startup **calibration succeeds** (delay measured, ~9-13 dB suppression, 0 errors) — better than the Pi HAT (monitor-zeros bug). The real blocker was a config bug: `get_bool`/`get_float` ignored the `JARVIS_<KEY>` env override (commit 4095c99) — fixed. No interface-abstraction or host `module-echo-cancel` needed for this path. | `utils/config_service.py`, `docker-compose.pulse.yaml`, `docs/container-audio.md` | M |

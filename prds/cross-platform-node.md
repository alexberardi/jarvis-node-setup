...
| **4 — Setup script + compose wiring** 🟡 | **Step 1 done** (91e30da): `docker-compose.audio.yaml` (`/dev/snd` + `group_add audio`), `scripts/configure-audio.sh` (enumerate via the image → pick → write `audio.env`), `docs/container-audio.md`. ALSA-direct path works on Ubuntu + Pi regardless of host PulseAudio; single-USB-device case needs no config. **Step 2 remaining:** shared host PulseAudio socket + non-root-uid rework (relocate `/root/.jarvis` mounts, mount pulse socket/cookie) for volume/duck/BT control; macOS→native steering. | `docker-compose.audio.yaml`, `scripts/configure-audio.sh`, `docs/container-audio.md`, `Dockerfile.audio` (USER), mount layout | M |

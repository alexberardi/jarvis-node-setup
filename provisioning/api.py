...
def _save_node_credentials(
    node_id: str, node_key: str, allow_overwrite: bool = False
) -> bool:
    """
    ...
    By default this refuses to overwrite an existing config that already has
    real (non-placeholder) credentials for a *different* node — a guard against
    stray/test calls clobbering a provisioned node. But a completed provisioning
    handshake is a deliberate (re)provision of THIS device, so it passes
    ``allow_overwrite=True``: without it, re-provisioning registers a new node
    with command-center but silently discards the new creds here, stranding the
    device on its old (now-deactivated) identity — 401 on every call, no MQTT.

    Args:
        node_id: The node ID
        node_key: The node's API key (generated by jarvis-auth)
        allow_overwrite: Overwrite existing real credentials. Set by the
            provisioning flow; leave False for incidental callers.
    ...
    try:
        ...
        # Guard: don't clobber a provisioned node from an incidental caller.
        # A real provisioning handshake passes allow_overwrite=True — re-provision
        # is a deliberate identity change for this device.
        _placeholders = {"your-node-id", "your_api_key_here", ""}
        existing_id = config.get("node_id", "")
        existing_key = config.get("api_key", "")
        if (
            not allow_overwrite
            and existing_id not in _placeholders
            and existing_key not in _placeholders
            and existing_id != node_id
        ):
            # Config already has real credentials for a different node — refuse
            _logger = logging.getLogger("provisioning")
            _logger.warning(
                "Refusing to overwrite existing node credentials in %s "
                "(existing node_id=%s, new node_id=%s). "
                "Use authorize_node.py --update-config to re-register.",
                config_path, existing_id[:8], node_id[:8],
            )
            return False
...
def _run_provisioning(
    ...
    try:
        ...
        # while it silently keeps stale (now-revoked) credentials.

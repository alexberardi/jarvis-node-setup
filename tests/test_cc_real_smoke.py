"""Smoke tests against a real `jarvis-command-center` running in
`docker-compose.ci.yaml`'s `core` profile.
...
don't bring up CC still pass these as "skipped" rather than failing.
"""
...
SKIP_REASON = "CC_URL unset — skipping real-CC smoke tests (v1 fakes-only mode)"
